...
from redisembeddings import execute_query, get_documents, set_document
from formrecognizer import analyze_read
from azureblobstorage import upload_file, upsert_blob_metadata
import tiktoken
...
def get_semantic_answer(df, question, explicit_prompt="", model="DaVinci-text", engine='babbage', limit_response=True, tokens_response=100, temperature=0.0):
    ...
    #print("after completion--->1",f"{response}\n\n\n")
    #print("after completion--->2",f"{response['choices'][0]['text'].encode().decode()}\n\n\n")
